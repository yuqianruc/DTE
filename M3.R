...
ptm=proc.time()
N=4000;d1=20;d2=500;tmax=500;K=5;M=floor(N/K)
logistic=function(x){exp(x)/(1+exp(x))}
func_link=function(x){(x>0)*abs(x)/(abs(x)+1)+(x<0)/(abs(x)+1)}
func=function(x){(x>-1)*(x<1)*(1-x)/2+(x<=-1)*x/x}
beta_pi1_0=0
beta_pi1_S1=5*c(rep(1,10),rep(0,d1-10))
beta_pi2_0_0=-2
beta_pi2_0_S1=-c(0.1,rep(0,d1-1))
beta_pi2_0_S2=-c(0.1,rep(0,d2-1))
beta_pi2_1_0=2
beta_pi2_1_S1=c(0.1,rep(0,d1-1))
beta_pi2_1_S2=c(0.1,rep(0,d2-1))
beta_g0_0=1
beta_g0_S2=0.3*c(0.9^(0:(d2-1)))
beta_g0_S1=rep(0,d1)
beta_g1_0=-1
beta_g1_S2=-0.3*c(0.9^(0:(d2-1)))
beta_g1_S1=rep(0,d1)
set.seed(123)
...
#S2_all=sign(W2_all)*abs(W2_all)^{1/2}
#S2_1_all=sign(W2_1_all)*abs(W2_1_all)^{1/2}
#S2_0_all=sign(W2_0_all)*abs(W2_0_all)^{1/2}
S2_all=abs(W2_all)^{1/2}
S2_1_all=abs(W2_1_all)^{1/2}
S2_0_all=abs(W2_0_all)^{1/2}
...
mu2_1_all=beta_g1_0+S1_all%*%beta_g1_S1+(matrix(rep(2*func(-S1_all_11)-1,d2),nrow=N*tmax,ncol=d2)*S2_1_all^2)%*%beta_g1_S2
mu2_0_all=beta_g0_0+S1_all%*%beta_g0_S1+(matrix(rep(2*func(-S1_all_10)-1,d2),nrow=N*tmax,ncol=d2)*S2_0_all^2)%*%beta_g0_S2
...
results_par <- foreach (t=1:tmax, .combine=rbind, .packages=c("glmnet")) %dopar% {
  ...
  for (k in 1:K){
    ...
    #pi2_1
    fit_Log2_1=cv.glmnet(x=cbind(S1[index1_nk,],S2[index1_nk,]),y=A2[index1_nk],family="binomial")
    pred_pi2_1[1,index]=predict(fit_Log2_1,newx=cbind(S1[index,],S2[index,]),type="response",s="lambda.min") #logistic
    #pi2_0
    fit_Log2_0=cv.glmnet(x=cbind(S1[index0_nk,],S2[index0_nk,]),y=A2[index0_nk],family="binomial")
    ...
    fit_Log2_1_new1=cv.glmnet(x=cbind(S1[index1_nk1,],S2[index1_nk1,]),y=A2[index1_nk1],family="binomial")
    ...
    fit_Log2_1_new2=cv.glmnet(x=cbind(S1[index1_nk2,],S2[index1_nk2,]),y=A2[index1_nk2],family="binomial")
    ...
    fit_Log2_0_new1=cv.glmnet(x=cbind(S1[index0_nk1,],S2[index0_nk1,]),y=A2[index0_nk1],family="binomial")
    ...
    fit_Log2_0_new2=cv.glmnet(x=cbind(S1[index0_nk2,],S2[index0_nk2,]),y=A2[index0_nk2],family="binomial")
  ...
  for (k in 1:K){
    ...
    #pi1
    fit_Log1=cv.glmnet(x=S1[index_nk,],y=A1[index_nk],family="binomial")
    pred_pi1_1[2,index]=predict(fit_Log1,newx=S1[index,],type="response",s="lambda.min") #logistic+lasso
    #pi2_1
    fit_Log2_1=cv.glmnet(x=cbind(S1[index1_nk,],S2[index1_nk,]),y=A2[index1_nk],family="binomial")
    pred_pi2_1[2,index]=predict(fit_Log2_1,newx=cbind(S1[index,],S2[index,]),type="response",s="lambda.min") #logistic
    #pi2_0
    fit_Log2_0=cv.glmnet(x=cbind(S1[index0_nk,],S2[index0_nk,]),y=A2[index0_nk],family="binomial")
    pred_pi2_0[2,index]=predict(fit_Log2_0,newx=cbind(S1[index,],S2[index,]),type="response",s="lambda.min") #logistic
    #mu2_1
    fit_lasso1=cv.glmnet(x=cbind(S1[index11_nk,],S2[index11_nk,]),y=Y[index11_nk],family="gaussian",alpha=1)
    pred_mu2_1[2,index]=predict(fit_lasso1,newx=cbind(S1[index,],S2[index,]),s="lambda.min") #lasso
    #mu2_0
    fit_lasso0=cv.glmnet(x=cbind(S1[index00_nk,],S2[index00_nk,]),y=Y[index00_nk],family="gaussian",alpha=1)
    pred_mu2_0[2,index]=predict(fit_lasso0,newx=cbind(S1[index,],S2[index,]),s="lambda.min") #lasso
    #mu1_1
  ...
  for (k in 1:K){
    ...
    #pi1
    fit_Log1=cv.glmnet(x=S1[index_nk,],y=A1[index_nk],family="binomial")
    pred_pi1_1[3,index]=predict(fit_Log1,newx=S1[index,],type="response",s="lambda.min") #logistic+lasso
    #pi2_1
    fit_Log2_1=cv.glmnet(x=cbind(S1[index1_nk,],S2[index1_nk,]),y=A2[index1_nk],family="binomial")
    pred_pi2_1[3,index]=predict(fit_Log2_1,newx=cbind(S1[index,],S2[index,]),type="response",s="lambda.min") #logistic
    #pi2_0
    fit_Log2_0=cv.glmnet(x=cbind(S1[index0_nk,],S2[index0_nk,]),y=A2[index0_nk],family="binomial")
    pred_pi2_0[3,index]=predict(fit_Log2_0,newx=cbind(S1[index,],S2[index,]),type="response",s="lambda.min") #logistic
    #mu2_1
    fit_lasso1=cv.glmnet(x=cbind(S1[index11_nk,],S2[index11_nk,]),y=Y[index11_nk],family="gaussian",alpha=1)
    pred_mu2_1[3,index]=predict(fit_lasso1,newx=cbind(S1[index,],S2[index,]),s="lambda.min") #lasso
    #mu2_0
    fit_lasso0=cv.glmnet(x=cbind(S1[index00_nk,],S2[index00_nk,]),y=Y[index00_nk],family="gaussian",alpha=1)
    pred_mu2_0[3,index]=predict(fit_lasso0,newx=cbind(S1[index,],S2[index,]),s="lambda.min") #lasso
    #mu1_1
    fit_lasso1_new1=cv.glmnet(x=cbind(S1[index11_nk,],S2[index11_nk,]),y=Y[index11_nk],family="gaussian",alpha=1)
    fit_Log2_1_new1=cv.glmnet(x=cbind(S1[index1_nk,],S2[index1_nk,]),y=A2[index1_nk],family="binomial")
    pred_mu2_1_new1=predict(fit_lasso1_new1,newx=cbind(S1[index1_nk,],S2[index1_nk,]),s="lambda.min")
    pred_pi2_1_new1=predict(fit_Log2_1_new1,newx=cbind(S1[index1_nk,],S2[index1_nk,]),type="response",s="lambda.min")
    Y1_new1=as.numeric(pred_mu2_1_new1+A2[index1_nk]*(Y[index1_nk]-pred_mu2_1_new1)/pred_pi2_1_new1)
    fit_lasso2_nocross1_new1=cv.glmnet(x=S1[index1_nk,],y=Y1_new1,family="gaussian",alpha=1)
    pred_mu1_1[3,index]=predict(fit_lasso2_nocross1_new1,newx=S1[index,],s="lambda.min")
    
    #mu1_0
    fit_lasso0_new1=cv.glmnet(x=cbind(S1[index00_nk,],S2[index00_nk,]),y=Y[index00_nk],family="gaussian",alpha=1)
    fit_Log2_0_new1=cv.glmnet(x=cbind(S1[index0_nk,],S2[index0_nk,]),y=A2[index0_nk],family="binomial")
    pred_mu2_0_new1=predict(fit_lasso0_new1,newx=cbind(S1[index0_nk,],S2[index0_nk,]),s="lambda.min")
    pred_pi2_0_new1=predict(fit_Log2_0_new1,newx=cbind(S1[index0_nk,],S2[index0_nk,]),type="response",s="lambda.min")
    Y0_new1=as.numeric(pred_mu2_0_new1+(1-A2[index0_nk])*(Y[index0_nk]-pred_mu2_0_new1)/(1-pred_pi2_0_new1))
    fit_lasso2_nocross0_new1=cv.glmnet(x=S1[index0_nk,],y=Y0_new1,family="gaussian",alpha=1)
    pred_mu1_0[3,index]=predict(fit_lasso2_nocross0_new1,newx=S1[index,],s="lambda.min")
  }
...
est_pi=factor(grid$est_pi,levels=1:num_piest,labels=c("log-DDRL-cross","log-DTL-nocross","log-DDRL'-nocross","oracle"))
est_mu1=factor(grid$est_mu1,levels=1:num_mu1est,labels=c("DDRL-cross","DTL-nocross","DDRL'-nocross","oracle"))
